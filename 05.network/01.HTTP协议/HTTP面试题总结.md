...
1. 客户端如果校验通过后，就根据证书的公钥的有效， 生成随机数，随机数使用公钥进行加密（RSA加密）；
1. 消息体产生的后，对它的摘要进行MD5（或者SHA1）算法加密，此时就得到了RSA签名；
1. 发送给服务端，此时只有服务端（RSA私钥）能解密。
1. 解密得到的随机数，再用AES加密，作为密钥（此时的密钥只有客户端和服务端知道）。
...
- [HTTP协议【详解】——经典面试题](http://www.cnblogs.com/sunny-sl/p/6529830.html)
